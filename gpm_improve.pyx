...
#    cdef float upper_y = RAND_MAX/(b- 2.0*m-1)
    ...
    cdef int iter_count, i, j, k, l, i1, j1, k1, new_rand, kk, z, i0, j0, k0, ii, jj, new_indx, k_min, k_max, j11, i11
...
#    cdef int w_loc_i= 5
    ...
    cdef int w_loc_k= min(w_loc_i/2, k_range)
...
#    cdef np.ndarray[np.int_t, ndim=2, negative_indices=False, mode='c'] dk_randy = np.random.randint(m, a-m, (a,b))
    ...
    for iter_count in xrange(max_iter):
        ...
        if iter_count == 0 and w_loc_i>1:
            ...
            for i in xrange(m , a-m):
                for j in xrange(m, b-m):
                    ...
                    k_mx= min(c-1, k_ref+w_loc_k+1)
                    
                    for i1 in xrange(i_mn, i_mx):
                        for j1 in xrange(j_mn, j_mx):
                            for k1 in xrange(k_mn, k_mx):
        ...
        if iter_count%2==0:
            ...
            for i in xrange(m+1 , a-m):
                for j in xrange(m+1 , b-m):
...
#        if i_rad<128 and iter_count%2==0:
#            i_rad*= 2
#            j_rad= i_rad
#            k_rad= i_rad/2
#            i_rad_2= i_rad/2
...
#        if n_rand_search<64 and iter_count%2==0:
#            n_rand_search*= 2
